...
wd = 'C:\experiments\Sammi\wmConfidence_eegfmri' #Anna Watts
...
participant = gui.Dlg(title="WM Confidence (Outside Scanner)", pos=(200,400));
...
subfname    = '%s/wmConfidence_eegfmri_outsidescanner_S%02d_info.csv' %(infofolder,subject['ID'])
subfile     = open(subfname,'w')
subfile.write('{},{},{},{},{} \n'.format(subject['ID'], subject['Age'], subject['Gender'], subject['handedness'], subject['date']))
subfile.close()
#
fname       = '%s/wmConfidence_eegfmri_outsidescanner_S%02d_%s.csv' %(behfolder,subject['ID'],subject['date'])
...
colcond = np.sort(np.tile(colcond, ntrls/2/colcond.size))
colcond = np.tile(colcond, ntrls/colcond.size)

cond  = np.tile(cond,  ntrls/cond.size)
cues  = np.tile(cues,  ntrls/cues.size)
pside = np.sort(np.tile(pside, ntrls/pside.size))
...
def runescape_task(block, blockDT, blockCT, blockResp, blockRespClicked, blockConfStartTime, blockConfClicked, blockConfClickTime, blockConfAngle, blockDialConfInterv, ntrls = 60): #quits the task, saving data up until the present point in time
    ...
    blockstruct.to_csv(op.join(behfolder, 'wmConfidence_eegfmri_outsidescanner_S%02d_block_%02d.csv'%(subject['ID'], block+1)), na_rep = np.nan, index = False)
    runescape()
...
fix = visual.Rect(screen, width = 1, height = 1, units = 'deg', fillColorSpace = 'rgb255', fillColor = colours[0], pos = (2.5,0))
...
def runTrial(trialnum, nframes, trlstruct):
    ...
    for i in np.add(np.arange(nframes, dtype = 'int'),1): #loop over frames, starting at 1
        ...
        if i == array_onset:
            ...
            fix.setAutoDraw(True) #used in pd test
        ...
        if i == array_offset:
            ...
            fix.setAutoDraw(False) #used in pd test
        ...
        if probe_started:                           #probe routine
            ...
            while not dialup:
                ...
                if count == (1*reshz): #half a second in frames for decision time
...
timinglog = logging.LogFile(f = op.join(logfolder, 'wmConfidence_eegfmri_outsidescanner_S%02d_logfile.txt'%(subject['ID'])), level = 22, filemode = 'a')
...
for block in range(nblocks):
    ...
    for i in range(ntrls):
        ...
        conffbdelayframes   = int(0.3         * reshz) #300ms delay between confidence ending and fb starting
...
struct.to_csv(op.join(behfolder, 'wmConfidence_eegfmri_outsidescanner_S%02d_allBlocks.csv'%(subject['ID'])), na_rep = np.nan, index = False)
