...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 30 15:02:44 2019
...
model = 'simple_effectsbycond'
model = 'simple_effectsbycond_withmain'

if model == 'simple_effectsbycond':
    contrasts = ['grandmean', 'cuedlvsr', 'neutrallvsr', 'dtcued', 'dtneut', 'errorcued', 'errorneut', 'confcued', 'confneut']
elif model == 'simple_effectsbycond_withmain':
    contrasts = ['grandmean', 'cuedlvsr', 'neutrallvsr','DT', 'error', 'confidence', 'dtcued', 'dtneut', 'errorcued', 'errorneut', 'confcued', 'confneut']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'probe','tfr_glm3',  model, 'wmc_' + param['subid'] + '_probelocked_tfr_'+ name + '_betas-tfr.h5'))[0])
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'probe','tfr_glm3',  model, 'wmc_' + param['subid'] + '_probelocked_tfr_'+ name + '_tstats-tfr.h5'))[0])
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'probe','tfr_glm3',  model, 'wmc_' + param['subid'] + '_probelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0])
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'probe','tfr_glm3',  model, 'wmc_' + param['subid'] + '_probelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0])
#%%
...
gave_cright = mne.grand_average(data_baselined_t['cuedright'])
gave_cright.data = toverparam(data_baselined_t['cuedright']); gave_cright.drop_channels(['RM'])
gave_cright.plot_joint(topomap_args = dict(outlines='head', contours=0, vmin=-4, vmax=4),
                      timefreqs = timefreqs_cue, title = 'cued right ' + model +' t over tstats')
...
gave_lvsr = mne.grand_average(data_t['cuedlvsr']); gave_lvsr.data = toverparam(data_t['cuedlvsr']); gave_lvsr.drop_channels(['RM'])
...
gave_error.plot_joint(topomap_args = dict(outlines='head', contours = 0, vmin = -3, vmax = 3),
                      timefreqs = timefreqs_cue, title = 'error ' +model+' t over tstats')
...
gave_error = mne.grand_average(data_t['errorcued']); gave_error.data = toverparam(data_t['errorcued'])
gave_error.drop_channels(['RM'])
gave_error.plot_joint(topomap_args = dict(outlines='head', contours = 0, vmin = -2, vmax = 2),
                      timefreqs = timefreqs_cue, title = 'error ' +model+' t over tstats')

gave_dt   = mne.grand_average(data_t['dtcued']); gave_dt.data =toverparam(data_t['dtcued']);
gave_dt.drop_channels(['RM'])
gave_dt.plot_joint(topomap_args = topoargs_t, timefreqs = timefreqs_cue, title = 'DT '+model+' t over tstats')

gave_conf = mne.grand_average(data_t['confcued']); gave_conf.data = toverparam(data_t['confcued'])
gave_conf.drop_channels(['RM'])
gave_conf.plot_joint(topomap_args = topoargs_t, timefreqs = timefreqs_cue, title = 'confidence '+model+' t over tstats')
...
cvsi_clvsr = np.subtract(np.nanmean(deepcopy(gave_conf).crop(tmin=-2,tmax=0).pick_channels(visrightchans).data,0),
                         np.nanmean(deepcopy(gave_conf).crop(tmin=-2,tmax=0).pick_channels(visleftchans).data, 0))
fig = plt.figure()
ax = fig.add_subplot(111)
ax.contourf(deepcopy(gave_lvsr).crop(tmin=-2,tmax=0).times, deepcopy(gave_lvsr).crop(tmin=-2,tmax=0).freqs, cvsi_clvsr, levels = 100, cmap = 'RdBu_r', antialiased = False, vmin = -2, vmax = 2)
ax.vlines([0, -1.5], linestyle = 'dashed', lw = .75, ymin=1, ymax = 39)
ax.set_title('confidence ~ cued lvsr, cvsi visual channels')
