...
contrasts = [#'grandmean',
             ...
             'cw_pleft_neutral','cw_pleft_cued','cw_pright_neutral','cw_pright_cued','cw_clvsn','cw_crvsn','cw_cued','cw_neutral','cw_clvsr','cw_cuedvsneut']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfrglm7_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0])
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfrglm7_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0])
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfrglm7_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0])
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfrglm7_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0])
#%%
timefreqs = {(.4, 10):(.4, 4),
             (.6, 10):(.4, 4),
             (.8, 10):(.4, 4),
...
vmin = -1e-10
vmax=vmin*-1
contrast = 'cw_cuedvsneut'
gave = mne.grand_average(data_baselined[contrast]); #gave.data = toverparam(data_baselined[contrast])
