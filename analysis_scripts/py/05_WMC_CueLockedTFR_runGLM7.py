...
#        tmpright = deepcopy(tfr)['cue==1 and pside==1'].average()
        ...
        conf        = np.multiply(cw, -1) #make this regressor indicate confidence, not confidence width.
        ...
        regressors.append(glm.regressors.ParametricRegressor(name = 'conf_pleft_neut', values = conf_npleft,  preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'conf_pleft_cued', values = conf_cleft,   preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'conf_pright_neut', values = conf_npright, preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'conf_pright_cued', values = conf_cright,  preproc = None, num_observations = nobs))
        ...
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 1, 0, 0, 0], 'conf_pleft_neutral')         )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0, 0], 'conf_pleft_cued')            )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0, 1, 0], 'conf_pright_neutral')        )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0, 0, 1], 'conf_pright_cued')           )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0,-1, 1, 0, 0], 'conf_clvsn'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0,-1, 1], 'conf_crvsn'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0,-1], 'conf_clvsr'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 1, 0, 1, 0], 'conf_neutral'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0, 1], 'conf_cued'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0,-1, 1,-1, 1], 'conf_cuedvsneut'))
...
#                                              topomap_args = dict(outlines='skirt', contours = 0),baseline=(-2,-1.5))
            tfr_betas.save(fname = op.join(param['path'], 'glms', 'cue', 'tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas' + addtopath + '-tfr.h5'), overwrite = True)
            del(tfr_betas)
...
#                                               topomap_args = dict(outlines='skirt', contours = 0, vmin=-4, vmax=4))#,baseline=(-2,-1.5))
            ...
            tfr_tstats.save(fname = op.join(param['path'], 'glms', 'cue', 'tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats' + addtopath + '-tfr.h5'), overwrite = True)
            del(tfr_tstats)
