...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 12 11:48:28 2019
...
for i in subs:
    ...
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 0, 1, 0], 'correct') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 0, 0, 1], 'incorrect') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 0,-1, 1], 'red vs green') ) #incorrect - correct (ERN localisation)
    contrasts.append( glm.design.Contrast([0, 0, 0, 1, 0, 1, 0], 'error correct') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 1, 0, 0, 1], 'error incorrect') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 1, 1, 0], 'conf correct') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 1, 0, 1], 'conf incorrect') )
    ...
    for iname in range(len(glmdes.contrast_names)):
        ...
        tl_betas.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm1', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_betas-ave.fif'))
        del(tl_betas)

        tl_tstats = mne.EvokedArray(info = info, nave = nave, tmin = tmin,
                                                  data = np.squeeze(model.get_tstats()[iname,:,:]))
        tl_tstats.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm1', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_tstats-ave.fif'))
        del(tl_tstats)

        tl_varcopes = mne.EvokedArray(info = info, nave = nave, tmin = tmin,
                                                  data = np.squeeze(model.varcopes[iname,:,:]))
        tl_varcopes.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm1', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_varcopes-ave.fif'))
        del(tl_varcopes)
