...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 12:51:57 2019
...
#        tmpright = deepcopy(tfr)['cue==1 and pside==1'].average()
        ...
        regressors.append(glm.regressors.ParametricRegressor(name = 'dt_pleft_neut', values = dt_npleft,  preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'dt_pleft_cued', values = dt_cleft,   preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'dt_pright_neut', values = dt_npright, preproc = None, num_observations = nobs))
        regressors.append(glm.regressors.ParametricRegressor(name = 'dt_pright_cued', values = dt_cright,  preproc = None, num_observations = nobs))
        ...
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 1, 0, 0, 0], 'dt_pleft_neutral')         )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0, 0], 'dt_pleft_cued')            )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0, 1, 0], 'dt_pright_neutral')        )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0, 0, 1], 'dt_pright_cued')           )
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0,-1, 1, 0, 0], 'dt_clvsn'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 0,-1, 1], 'dt_crvsn'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0,-1], 'dt_clvsr'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 1, 0, 1, 0], 'dt_neutral'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0, 0, 1, 0, 1], 'dt_cued'))
        contrasts.append(glm.design.Contrast([  0, 0, 0, 0,-1, 1,-1, 1], 'dt_cuedvsneut'))
...
#                                              topomap_args = dict(outlines='skirt', contours = 0),baseline=(-2,-1.5))
            tfr_betas.save(fname = op.join(param['path'], 'glms', 'cue', 'tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas' + addtopath + '-tfr.h5'), overwrite = True)
            del(tfr_betas)
...
#                                               topomap_args = dict(outlines='skirt', contours = 0, vmin=-4, vmax=4))#,baseline=(-2,-1.5))
            ...
            tfr_tstats.save(fname = op.join(param['path'], 'glms', 'cue', 'tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats' + addtopath + '-tfr.h5'), overwrite = True)
            del(tfr_tstats)
