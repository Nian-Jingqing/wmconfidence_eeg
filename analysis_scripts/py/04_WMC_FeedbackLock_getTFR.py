...
for i in sublist:
    ...
    fblocked.resample(250) #downsample to 250hz so don't overwork the workstation lols
    if i == 10: #not sure why, need to check, but metadata not in this datafile...
        fblocked.metadata = pd.DataFrame.from_csv(path = param['behaviour_blinkchecked'], index_col = None)
