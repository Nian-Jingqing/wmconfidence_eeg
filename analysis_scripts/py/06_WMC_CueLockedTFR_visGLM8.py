...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 22 17:40:59 2019
...
contrasts = ['pleft_cued', 'pleft_neutral', 'pright_cued', 'pright_neutral', 'neutral', 'cued', 'crvsn', 'clvsn','clvsr', 'cuedvsneut',
             'pterr_pleft_neutral','pterr_pleft_cued','pterr_pright_neutral','pterr_pright_cued',
             'pterr_clvsn','pterr_crvsn','pterr_cued','pterr_neutral','pterr_clvsr','pterr_cuedvsneut', 'pterr_nlvsr']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfr_glm8', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0])
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfr_glm8', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0])
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfr_glm8', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0])
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfr_glm8', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0])
#%%
timefreqs = {(.4, 10):(.4, 4),
             (.6, 10):(.4, 4),
             (.8, 10):(.4, 4),
...
vmin = -5e-11; #vmin = -3
vmax=vmin*-1
contrast = 'pterr_clvsr'
gave = mne.grand_average(data[contrast]); #gave.data = toverparam(data[contrast])
gave.plot_joint(topomap_args = dict(outlines='skirt', contours=0,vmin=vmin,vmax=vmax), #baseline = (-1.75,-1.25), title ='baselined',#vmin=-4,vmax=4),
                timefreqs = timefreqs_alpha, vmin=vmin, vmax=vmax)
...
#    rhs_chaninds = 
    ...
    if contrast in ['clvsr', 'pterr_clvsr']:
...
#    tmp.data[midline_chaninds,:,:] = 0
    ...
    axes = fig.axes
    axes[0].clear()
    if contrast in ['clvsr','pleft_cued','clvsn', 'pterr_clvsr', 'pterr_clvsn']:
        chans = visrightchans
