...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 12:04:09 2019
...
contrasts = ['pleft_neutral','pleft_cued','pright_neutral','pright_cued',
             ...
             'conf_clvsn','conf_crvsn','conf_clvsr',
             'conf_neutral','conf_cued','conf_cuedvsneut']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfrglm7', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
...
stat = 'tstat'
baselined = False
contrast = 'conf_pleft_cued'
for contrast in ['conf_pleft_cued', 'conf_pright_cued', 'conf_cued', 'conf_clvsn', 'conf_crvsn', 'conf_clvsr']:
    if contrast in ['conf_pleft_cued', 'conf_pright_cued', 'conf_cued']:
        baselined = True
    else:
        baselined = False
    ...
    if contrast in ['conf_pleft_cued','conf_clvsn','conf_clvsr']:
        plot_lvsr = True
    else:
        plot_lvsr = True
...
#key contrast of interest here is confidence in cued left vs right
#a negative beta/tstat would mean that lower power is associated with higher confidence
#for the cued left vs right contrast, if there was item specific selection that related to confidence
#we would expect to see that the beta/tstat is lower over right visual channels than left visual channels

stat = 'tstat'
baselined = True
contrast = 'conf_clvsr'
for contrast in ['conf_clvsn', 'conf_crvsn', 'conf_clvsr']:
    for baselined in [True, False]:
        ...
        if contrast in ['conf_clvsr', 'conf_clvsn']:
            contrachans = visrightchans
            ipsichans   = visleftchans
        elif contrast in ['conf_crvsn']:
