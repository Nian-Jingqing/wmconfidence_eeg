...
for i in sublist:
    ...
    cuelocked.resample(250) #downsample to 250hz so don't overwork the workstation lols
    if i == 10: #not sure why, need to check, but metadata not in this datafile...
        cuelocked.metadata = pd.DataFrame.from_csv(path = param['behaviour_blinkchecked'])
