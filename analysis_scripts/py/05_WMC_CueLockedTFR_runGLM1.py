...
#subs = np.array([         12, 13, 14, 15, 16, 17, 18,     20, 21, 22,     24, 25, 26])
...
for i in subs:
    for iglm in range(glms2run):
        ...
        for laplacian in [True, False]:
            ...
            #get tfr
            if laplacian:
                tfr = mne.time_frequency.read_tfrs(fname = param['cuelocked_tfr'].replace('cuelocked-tfr', 'cuelocked_laplacian_stiffness4-tfr')); tfr = tfr[0]; #read in data with surface laplacian filter applied to epoched data
            else:
    ...
    #        tmpright = deepcopy(tfr)['cue==1 and pside==1'].average()
            ...
            contrasts.append(glm.design.Contrast([  1, 0,-1, 0], 'nlvsn')) #10
            contrasts.append(glm.design.Contrast([ -1, 1, 1,-1], 'clvsrVSnlvsr')) #11
