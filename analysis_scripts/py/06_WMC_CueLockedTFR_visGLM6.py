...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 15 11:26:53 2019
...
contrasts = [#'grandmean',
             'pleft_cued', 'pleft_neutral', 'pright_cued', 'pright_neutral', 'neutral', 'cued', 'crvsn', 'clvsn','clvsr', 'cuedvsneutral',
             'pln_above', 'pln_below', 'plc_above', 'plc_below', 'prn_above', 'prn_below', 'prc_above', 'prc_below',
             'cleft_belowvsabove','creft_belowvsabove','clvsr_belowvsabove']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfrglm6_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0])
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfrglm6_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0])
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfrglm6_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0])
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfrglm6_nogmean', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0])
#%%
timefreqs = {(.4, 10):(.4, 4),
             (.6, 10):(.4, 4),
             (.8, 10):(.4, 4),
...
#        data_baselined_t[contrast][i].drop_channels(['RM'])
        ...
        data[contrast][i].crop(tmin=-0.7)
        data_t[contrast][i].crop(tmin=-0.7)
        data_baselined[contrast][i].crop(tmin=-0.7)
        data_baselined[contrast][i].crop(tmin=-0.7)
...
gave = mne.grand_average(data_t[contrast]); gave.data = toverparam(data_t[contrast])
gave.plot_joint(topomap_args = dict(outlines='skirt', contours=0, vmin=vmin, vmax=vmax),#,vmin=-3,vmax=3), #baseline = (-1.75,-1.25), title ='baselined',#vmin=-4,vmax=4),
                timefreqs = timefreqs_alpha, vmin=vmin, vmax=vmax)#, vmin=-3, vmax=3)
...
#cleft_below = mne.grand_average(data['prc_below'])
#cleft_above = mne.grand_average(data['prc_above'])           
 
cleft_bvsa  = mne.grand_average(data_baselined_t['cleft_belowvsabove']); cleft_bvsa.data = toverparam(data_baselined_t['cleft_belowvsabove'])
cright_bvsa  = mne.grand_average(data_baselined_t['creft_belowvsabove']); cright_bvsa.data = toverparam(data_baselined_t['creft_belowvsabove'])
clvsr_bvsa = mne.grand_average(data_t['clvsr_belowvsabove']); clvsr_bvsa.data = toverparam(data_t['clvsr_belowvsabove'])


#alpha_cleftb = deepcopy(cleft_below).crop(fmin=8,fmax=12)
#alpha_clefta = deepcopy(cleft_above).crop(fmin=8,fmax=12) 
         
alpha_cleftbvsa = deepcopy(cleft_bvsa).crop(fmin=8,fmax=12).crop(tmin=-0.7) 
alpha_crightbvsa = deepcopy(cright_bvsa).crop(fmin=8,fmax=12).crop(tmin=-0.7)     
alpha_clvsrbvsa = deepcopy(clvsr_bvsa).crop(fmin=8,fmax=12).crop(tmin=-0.7)     
              
#alpha_cleftb_cvsi = np.subtract(
#        np.nanmean(np.nanmean(deepcopy(alpha_cleftb).pick_channels(visrightchans).data,0),0),
#        np.nanmean(np.nanmean(deepcopy(alpha_cleftb).pick_channels(visleftchans).data,0),0),
#        )
#              
#alpha_clefta_cvsi = np.subtract(
#        np.nanmean(np.nanmean(deepcopy(alpha_clefta).pick_channels(visrightchans).data,0),0),
#        np.nanmean(np.nanmean(deepcopy(alpha_clefta).pick_channels(visleftchans).data,0),0),
#        )         

alpha_cleftbvsa_cvsi = np.subtract(
        np.nanmean(np.nanmean(deepcopy(alpha_cleftbvsa).pick_channels(visrightchans).data,0),0),
        np.nanmean(np.nanmean(deepcopy(alpha_cleftbvsa).pick_channels(visleftchans).data,0),0),
        )
alpha_crightbvsa_cvsi = np.subtract(
        np.nanmean(np.nanmean(deepcopy(alpha_crightbvsa).pick_channels(visleftchans).data,0),0),
        np.nanmean(np.nanmean(deepcopy(alpha_crightbvsa).pick_channels(visrightchans).data,0),0),
        )    

alpha_clvsrbvsa_cvsi = np.subtract(
        np.nanmean(np.nanmean(deepcopy(alpha_clvsrbvsa).pick_channels(visrightchans).data,0),0),
        np.nanmean(np.nanmean(deepcopy(alpha_clvsrbvsa).pick_channels(visleftchans).data,0),0),
        )          
...
fig, ax = plt.subplots()
ax.plot(alpha_clvsrbvsa.times, alpha_cleftbvsa_cvsi, lw = 1, label = 'cued left, cvsi, below median error')             
ax.plot(alpha_clvsrbvsa.times, alpha_crightbvsa_cvsi, lw = 1, label = 'cued right, cvsi, above median error')  
ax.plot(alpha_clvsrbvsa.times, alpha_clvsrbvsa_cvsi, lw = 1, label = 'cued lvsr, cvsi, below vs above median error')  

ax.hlines([0], color = '#000000', linestyles='dashed', lw = .5, xmin = alpha_clvsrbvsa.times.min(), xmax = alpha_clvsrbvsa.times.max())
ax.legend()
