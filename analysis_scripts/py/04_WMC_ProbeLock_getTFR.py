...
for i in sublist:
    ...
    probelocked.resample(250) #downsample to 250hz so don't overwork the workstation lols
    if i == 10: #not sure why, need to check, but metadata not in this datafile...
        probelocked.metadata = pd.DataFrame.from_csv(path = param['behaviour_blinkchecked'])
