...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 16:03:14 2019
...
for channel in ['Cz']:
    mne.viz.plot_compare_evokeds(
            evokeds = dict(defcorr   = data['confdefcorrect'],
            ...
            ci = .68, show_sensors = False, title = 'confidence regressor at electrode ' + channel,
...
for channel in ['FCz', 'Cz', 'CPz', 'Pz']:
    mne.viz.plot_compare_evokeds(
            evokeds = dict(incorrvsdef    = data['confincorrvsdef'],
                           incorrvsjust   = data['confincorrvsjust'],
                           justvsdef      = data['confjustvsdef']),
            colors = dict(incorrvsjust    = '#8da0cb',
                          incorrvsdef     = '#fc8d62',
                          justvsdef       = '#66c2a5'),
#            linestyles = dict(justcorr    = 'dashed'),
            show_legend = 'upper right', picks = channel,
            ci = .68, show_sensors = False, title = 'difference in confidence regressor between trial types at electrode ' + channel,
            truncate_xaxis=False)            
...
for channel in ['FCz', 'Cz', 'Pz']:
    ...
    t_ivsj, clu_ivsj, clupv_ivsj, _ = runclustertest_epochs(data = dat2use,
                                                            contrast_name = 'confincorrvsjust',
                                                            channels = [channel],
                                                            tmin = tmin,
                                                            tmax = tmax,
                                                            gauss_smoothing = None, out_type = 'indices',
    ...
    #difference in confidence regressor between incorrect and definitely correct trials
    plotdat_ivsd = np.empty(shape = (subs.size, plottimes.size))
    for i in range(subs.size):
        plotdat_ivsd[i,:] = np.squeeze(deepcopy(dat2use['confincorrvsdef'][i]).pick_channels([channel]).crop(tmax=tmax).data)
    
    plotdatmean_ivsd = np.nanmean(plotdat_ivsd, axis = 0)
    plotdatsem_ivsd  = sp.stats.sem(plotdat_ivsd, axis = 0)
    
    
    #difference in confidence regressor between incorrect trials and just correct trials
    plotdat_ivsj = np.empty(shape = (subs.size, plottimes.size))
    for i in range(subs.size):
        plotdat_ivsj[i,:] = np.squeeze(deepcopy(dat2use['confincorrvsjust'][i]).pick_channels([channel]).crop(tmax=tmax).data)
    
    plotdatmean_ivsj = np.nanmean(plotdat_ivsj, axis = 0)
    plotdatsem_ivsj  = sp.stats.sem(plotdat_ivsj, axis = 0)
    
    #difference in cofnidence regressor between just correct and definitely correct trials
    plotdat_jvsd = np.empty(shape = (subs.size, plottimes.size))
    for i in range(subs.size):
        plotdat_jvsd[i,:] = np.squeeze(deepcopy(dat2use['confjustvsdef'][i]).pick_channels([channel]).crop(tmax=tmax).data)
...
#    fig.savefig(fname = op.join(figpath, 'confidence_x_diffwaves_20subs_threshold%d_channel_%s_%s.eps'%(abs(threshold),channel, stat)), format = 'eps', dpi = 300)
#    fig.savefig(fname = op.join(figpath, 'confidence_x_diffwaves_20subs_threshold%d_channel_%s_%s.pdf'%(abs(threshold),channel, stat)), format = 'pdf', dpi = 300)
#    plt.close()
#%%

# we know there are differences in confidence across the trial types
# so lets loop over these trial types, focussing on the confidence regressor (this helps to describe these differences between trial types)
stat = 'beta' #choose whether you want to use the single subject tstats or betas for the analysis
if stat == 'beta':
    dat2use = deepcopy(data)
elif stat == 'tstat':
...
#for contrast in ['confdefcorrect', 'confjustcorrect', 'confincorrect']:
    tmin = 0
    tmax = 1
    # there is a cluster in the incorrect trials that comes out if the tmax is set to 750ms
    # (250ms after offset of the feedback stimulus) rather than the full 1second period
    clutimes = deepcopy(dat2use['incorrect'][0]).crop(tmin=tmin,tmax=tmax).times
    ...
    for i in range(subs.size):
        plotdat_def[i,:] = np.squeeze(deepcopy(dat2use['confdefcorrect'][i]).pick_channels([channel]).crop(tmax=tmax).data)
    ...
    for i in range(subs.size):
        plotdat_just[i,:] = np.squeeze(deepcopy(dat2use['confjustcorrect'][i]).pick_channels([channel]).crop(tmax=tmax).data)
    ...
    fig = plt.figure(figsize = (12, 6))
    ax  = fig.add_subplot(111)
    ax.plot(plottimes, plotdatmean_def, color = '#31a354', lw = 1.5, label = 'confidence regressor: definitely correct')
    ax.fill_between(plottimes, plotdatmean_def - plotdatsem_def, plotdatmean_def + plotdatsem_def, alpha = .3, color = '#31a354')
    
    ax.plot(plottimes, plotdatmean_just, color = '#a1d99b', lw = 1.5, label = 'confidence regressor: just correct', ls = 'dashed')
    ax.fill_between(plottimes, plotdatmean_just - plotdatsem_just, plotdatmean_just + plotdatsem_just, alpha = .3, color = '#a1d99b')
    
    ax.plot(plottimes, plotdatmean_incorr, color = '#de2d26', lw = 1.5, label = 'confidence regressor: incorrect')
    ...
    fig.savefig(fname = op.join(figpath, 'confidence_trialtypes_20subs_threshold%d_channel_%s_%s.eps'%(abs(threshold), channel, stat)), format = 'eps', dpi = 300)
    fig.savefig(fname = op.join(figpath, 'confidence_trialtypes_20subs_threshold%d_channel_%s_%s.pdf'%(abs(threshold), channel, stat)), format = 'pdf', dpi = 300)
#    plt.close()
#%%
...
for contrast in ['confincorrvsdef', 'confincorrvsjust', 'confjustvsdef', 'confdefcorrect', 'confjustcorrect', 'confincorrect']:
    for channel in ['FCz', 'Cz', 'Pz']:
        ...
        print('contrast %s: %s significant clusters'%(contrast, str(len(masks_cope))))
        ...
        vmin, vmax = -4, 4 #microvolts for the min/max values of the topographies we're going to plot
        if contrast == 'confdefcorrect':
            vmin,vmax = -1,1
