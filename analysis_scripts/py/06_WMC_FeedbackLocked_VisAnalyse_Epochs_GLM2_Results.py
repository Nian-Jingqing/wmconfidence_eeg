...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 12 22:24:25 2019
...
for i in subs:
    ...
    alldata_grandmean.append(mne.read_evokeds(fname       = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_grandmean_betas-ave.fif'))[0])
    alldata_neutral.append(mne.read_evokeds(fname         = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_neutral_betas-ave.fif'))[0])
    alldata_cued.append(mne.read_evokeds(fname            = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_cued_betas-ave.fif'))[0])
    alldata_error.append(mne.read_evokeds(fname           = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_error_betas-ave.fif'))[0])
    alldata_conf.append(mne.read_evokeds(fname            = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_confidence_betas-ave.fif'))[0])
    alldata_ERNxerror.append(mne.read_evokeds(fname       = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_incorrvscorrxerror_betas-ave.fif'))[0])
    alldata_ERNxconf.append(mne.read_evokeds(fname        = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_incorrvscorrxconf_betas-ave.fif'))[0])


    alldata_grandmean_t.append(mne.read_evokeds(fname       = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_grandmean_tstats-ave.fif'))[0])
    alldata_neutral_t.append(mne.read_evokeds(fname         = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_neutral_tstats-ave.fif'))[0])
    alldata_cued_t.append(mne.read_evokeds(fname            = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_cued_tstats-ave.fif'))[0])
    alldata_error_t.append(mne.read_evokeds(fname           = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_error_tstats-ave.fif'))[0])
    alldata_conf_t.append(mne.read_evokeds(fname            = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_confidence_tstats-ave.fif'))[0])
    alldata_ERNxerror_t.append(mne.read_evokeds(fname       = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_incorrvscorrxerror_tstats-ave.fif'))[0])
    alldata_ERNxconf_t.append(mne.read_evokeds(fname        = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_incorrvscorrxconf_tstats-ave.fif'))[0])

#%%

gave_error = mne.grand_average(alldata_error); gave_error.drop_channels(['RM'])
gave_error.plot_joint(picks = 'eeg',topomap_args = dict(outlines='head', contours =0), title ='glm2 main effect error')




#%%
# Question 1 - is the ERN (incorrect vs correct trials difference) modulated by error

gave_ernerror = mne.grand_average(alldata_ERNxerror); gave_ernerror.drop_channels(['RM'])

mne.viz.plot_compare_evokeds(
        evokeds = dict(
                error_ern = alldata_ERNxerror,
                conf_ern  = alldata_ERNxconf),
        colors = dict(
                error_ern = '#b2df8a',
                conf_ern  = '#1f78b4'),
        ci = .68,
...
gave_ernerror.plot_joint(picks = 'eeg', topomap_args = dict(outlines = 'head', contours = 0), title = 'incorrect vs correct x conf')

gave_ernconf = mne.grand_average(alldata_ERNxconf); gave_ernconf.drop_channels(['RM'])
gave_ernconf.plot_joint(picks = 'eeg', topomap_args = dict(outlines = 'head', contours = 0), title = 'incorrect vs correct x conf')
