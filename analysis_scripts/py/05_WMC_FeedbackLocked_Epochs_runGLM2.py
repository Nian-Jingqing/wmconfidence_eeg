...
for i in subs:
    ...
    incorrvscorr = np.where(epochs.metadata.confdiff.to_numpy() <= 0, -1, 1)
    #np.where(targinconf == 1, -1, 1) #this is just the contrast of the two but we can set it up as a contrast anyway, just here in case
    ...
    #regressors.append(glm.regressors.ParametricRegressor(name = 'ERN x error', values = np.multiply(error, incorrvscorr), preproc = 'z', num_observations = glmdata.num_observations))
    #regressors.append(glm.regressors.ParametricRegressor(name = 'ERN x conf',  values = np.multiply(conf, incorrvscorr), preproc = 'z', num_observations = glmdata.num_observations))
    ...
    contrasts.append( glm.design.Contrast([0, 0, 0, 1, 0, 0], 'error') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 1, 0], 'confidence') )
    contrasts.append( glm.design.Contrast([0, 0, 0, 0, 0, 1], 'incorr vs corr') )
    ...
    for iname in range(len(glmdes.contrast_names)):
        ...
        tl_betas.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_betas-ave.fif'))
        del(tl_betas)

        tl_tstats = mne.EvokedArray(info = info, nave = nave, tmin = tmin,
                                                  data = np.squeeze(model.get_tstats()[iname,:,:]))
        tl_tstats.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_tstats-ave.fif'))
        del(tl_tstats)

        tl_varcopes = mne.EvokedArray(info = info, nave = nave, tmin = tmin,
                                                  data = np.squeeze(model.varcopes[iname,:,:]))
        tl_varcopes.save(fname = op.join(param['path'], 'glms', 'feedback', 'epochs_glm2', 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ name + '_varcopes-ave.fif'))
        del(tl_varcopes)
