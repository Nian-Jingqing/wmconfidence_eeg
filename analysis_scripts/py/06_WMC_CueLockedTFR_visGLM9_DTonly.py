...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 13:55:29 2019
...
contrasts = ['pleft_neutral','pleft_cued','pright_neutral','pright_cued',
             ...
             'dt_clvsn','dt_crvsn','dt_clvsr',
             'dt_neutral','dt_cued','dt_cuedvsneut']
...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfrglm9', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0].crop(tmin=-0.5,tmax=None))
...
stat = 'tstat'
baselined = False
contrast = 'dt_pleft_cued'
for contrast in ['dt_pleft_cued', 'dt_pright_cued', 'dt_cued', 'dt_clvsn', 'dt_crvsn', 'dt_clvsr']:
    if contrast in ['dt_pleft_cued', 'dt_pright_cued', 'dt_cued']:
        baselined = True
    else:
        baselined = False
    ...
    if contrast in ['dt_pleft_cued','dt_clvsn','dt_clvsr']:
        plot_lvsr = True
    else:
        plot_lvsr = True
...
#key contrast of interest here is DT in cued left vs right
#a positive beta/tstat would mean that lower power is associated with lower (faster) DT
#for the cued left vs right contrast, if there was item specific selection that related to DT
#we would expect to see that the beta/tstat is higher over right visual channels than left visual channels

stat = 'tstat'
baselined = False
contrast = 'dt_clvsr'
for contrast in ['dt_clvsn', 'dt_crvsn', 'dt_clvsr']:
    for baselined in [True, False]:
        ...
        if contrast in ['dt_clvsr', 'dt_clvsn']:
            contrachans = visrightchans
            ipsichans   = visleftchans
        elif contrast in ['dt_crvsn']:
...
fig.suptitle('%s trials'%(contrast))
