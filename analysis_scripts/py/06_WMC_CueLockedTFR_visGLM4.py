...
for i in subs:
    ...
    for name in contrasts:
        data[name].append(mne.time_frequency.read_tfrs(            fname = op.join(param['path'], 'glms', 'cue','tfrglm4', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas-tfr.h5'))[0])
        data_t[name].append(mne.time_frequency.read_tfrs(          fname = op.join(param['path'], 'glms', 'cue','tfrglm4', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats-tfr.h5'))[0])
        data_baselined[name].append(mne.time_frequency.read_tfrs(  fname = op.join(param['path'], 'glms', 'cue','tfrglm4', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_betas_baselined-tfr.h5'))[0])
        data_baselined_t[name].append(mne.time_frequency.read_tfrs(fname = op.join(param['path'], 'glms', 'cue','tfrglm4', 'wmc_' + param['subid'] + '_cuelocked_tfr_'+ name + '_tstats_baselined-tfr.h5'))[0])
#%%
timefreqs = {(.4, 10):(.4, 4),
             (.6, 10):(.4, 4),
             (.8, 10):(.4, 4),
...
gave.plot_joint(topomap_args = dict(outlines='skirt', contours=0), #baseline = (-1.75,-1.25), title ='baselined',#vmin=-4,vmax=4),
...
for i in range(subs.size):
    ...
    cvsi[i,:,:] = np.subtract(
        np.nanmean(deepcopy(data_t['clvsr'])[i].pick_channels(visrightchans).data,0),
        np.nanmean(deepcopy(data_t['clvsr'])[i].pick_channels(visleftchans).data,0)
        )

#gave = mne.grand_average(data['clvsr'])
#cvsi = np.subtract(
#        np.nanmean(deepcopy(gave).pick_channels(visrightchans).data,0),
#        np.nanmean(deepcopy(gave).pick_channels(visleftchans).data,0)
#        )

cvsi = sp.stats.ttest_1samp(cvsi, axis=0, popmean=0)[0]


fig=plt.figure()
ax=fig.add_subplot(111)
tfplot = ax.imshow(cvsi, cmap='RdBu_r',aspect='auto', interpolation = 'hanning', origin = 'lower',vmin=-3,vmax=3,
          extent = (np.min(times), np.max(times), np.min(allfreqs), np.max(allfreqs)   ))
