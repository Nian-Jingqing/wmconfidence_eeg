...
#    fig.savefig(fname = op.join(figpath, 'feedbackERP_errorRegressor_electrode_%s_clustertimes_betas.eps'%(channel)), format = 'eps', dpi = 300 )
#    fig.savefig(fname = op.join(figpath, 'feedbackERP_errorRegressor_electrode_%s_clustertimes_betas.pdf'%(channel)), format = 'pdf', dpi = 300 )
...
for channel in ['FCz', 'Cz']:
    ...
    fig.savefig(fname = op.join(figpath, 'feedbackERP_errorRegressor_clustertimesFrom_%s_betas.eps'%(channel)), format = 'eps', dpi = 300 )
    fig.savefig(fname = op.join(figpath, 'feedbackERP_errorRegressor_clustertimesFrom_%s_betas.pdf'%(channel)), format = 'pdf', dpi = 300 )
...
for channel in ['FCz', 'Cz']:
    ...
    for imask in range(len(tmins)):
        ...
        fig.savefig(fname = op.join(figpath, 'errorCorrect_cluster_%sms_to_%sms_betas_fromChannel_%s.eps'%(str(np.round(itmin,2)), str(np.round(itmax,2)), channel)), format = 'eps', dpi = 300 )
        fig.savefig(fname = op.join(figpath, 'errorCorrect_cluster_%sms_to_%sms_betas_fromChannel_%s.pdf'%(str(np.round(itmin,2)), str(np.round(itmax,2)), channel)), format = 'pdf', dpi = 300 )
