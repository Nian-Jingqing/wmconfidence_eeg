...
for mask in masks_cope:
    ...
    fig.savefig(fname = op.join(figpath, 'feedbackTFR_sigCluster_%s_%sms_to_%ssms_thetafreqs.eps'%(str(np.squeeze(channel)), str(np.round(itmin,2)), str(np.round(itmax,2)))), format = 'eps', dpi = 300)
    fig.savefig(fname = op.join(figpath, 'feedbackTFR_sigCluster_%s_%sms_to_%ssms_thetafreqs.pdf'%(str(np.squeeze(channel)), np.round(itmin,2), np.round(itmax,2))), format = 'eps', dpi = 300)
