...
tfplot = ax.imshow(tfdata,
                   cmap = 'RdBu_r', aspect = 'auto', vmin = -3, vmax = 3, interpolation = 'none',
...
plt.tight_layout()
fig.savefig(fname = op.join(figpath,'feedbackTFR_incorrvsCorr_channel_%s_laplacian.eps'%(str(np.squeeze(channel)))), format = 'eps', dpi = 300)
fig.savefig(fname = op.join(figpath,'feedbackTFR_incorrvsCorr_channel_%s_laplacian.pdf'%(str(np.squeeze(channel)))), format = 'pdf', dpi = 300)
...
for mask in masks_cope:
    ...
    fig.savefig(fname = op.join(figpath, 'feedbackTFR_sigCluster_%s_%sms_to_%ssms_thetafreqs_laplacian.eps'%(str(np.squeeze(channel)), str(np.round(itmin,2)), str(np.round(itmax,2)))), format = 'eps', dpi = 300)
    fig.savefig(fname = op.join(figpath, 'feedbackTFR_sigCluster_%s_%sms_to_%ssms_thetafreqs_laplacian.pdf'%(str(np.squeeze(channel)), np.round(itmin,2), np.round(itmax,2))), format = 'eps', dpi = 300)
