...
contrasts = ['grandmean', 'neutral', 'cued',
             ...
             'errornleft', 'errorcleft', 'errornright', 'errorcright',
             'errorcued', 'errorneutral',
             'errorclvsr', 'errorclvsn', 'errorcrvsn', 'errornlvsr']
...
for contrast in ['errorcleft', 'errorcright','errorclvsr']:#,'errornlvsr', 'errorcued', 'errorclvsr']:
...
clvsr  = mne.grand_average(deepcopy(data)['errorclvsr'])
cleft  = mne.grand_average(deepcopy(data)['errorcleft'])
cright = mne.grand_average(deepcopy(data)['errorcright']) 
clvsr.plot_joint()
...
plot_po7_mean = np.nanmean(allpo7, axis = 0)
plot_po7_sem  = sp.stats.sem(allpo7, axis = 0)

plot_po8_mean = np.nanmean(allpo8, axis = 0)
plot_po8_sem  = sp.stats.sem(allpo8, axis = 0)

diff_mean = np.nanmean(np.subtract(allpo7, allpo8), axis = 0)
diff_sem  = sp.stats.sem(np.subtract(allpo7, allpo8), axis = 0)
