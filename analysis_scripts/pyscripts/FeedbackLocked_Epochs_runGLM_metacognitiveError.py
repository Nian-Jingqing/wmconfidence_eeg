# -*- coding: utf-8 -*-
"""
Created on Wed Oct 21 12:45:22 2020
...
for i in subs:
    ...
    #    epochs.set_eeg_reference(['RM']) #don't need this as the re-referencing is done in the combine session script, after concatenation and before trial rejection
        ...
        if model == 1:
            ...
            confdiff = np.radians(epochs.metadata.confdiff.to_numpy()) #error awareness (metacognition) on that trial (or prediction error)
            # deviation of target from confidence interval edge
            confdiff = nanzscore(confdiff)
            
            regressors.append(glm.regressors.ParametricRegressor(name = 'confdiff', values = confdiff, preproc = None, num_observations = nobs))
            
            contrasts = list()
            contrasts.append(glm.design.Contrast([ 1, 0, 0], 'correct'))
            contrasts.append(glm.design.Contrast([ 0, 1, 0], 'incorrect'))
            contrasts.append(glm.design.Contrast([ 0, 0, 1], 'confdiff'))
            contrasts.append(glm.design.Contrast([ 1, 1, 0], 'grandmean'))
            contrasts.append(glm.design.Contrast([-1, 1, 0], 'incorrvscorr'))
        ...
        if model == 2:
            ...
            #separate by condition (correct and incorrect)
            confdiff_correct   = np.where(np.less_equal(confdiff,0), confdiff, np.nan)
            confdiff_incorrect = np.where(np.greater(confdiff,0), confdiff, np.nan)
            
            confdiff_correct   = nanzscore(confdiff_correct)
            confdiff_incorrect = nanzscore(confdiff_incorrect)
            
            regressors = list()
            regressors.append(glm.regressors.CategoricalRegressor(category_list = targinconf,      codes = 1, name = 'correct'))
            regressors.append(glm.regressors.CategoricalRegressor(category_list = targoutsideconf, codes = 1, name = 'incorrect'))
            regressors.append(glm.regressors.ParametricRegressor(name = 'confdiff-correct',   values = confdiff_correct,   preproc = None, num_observations = nobs))
            regressors.append(glm.regressors.ParametricRegressor(name = 'confdiff-incorrect', values = confdiff_incorrect, preproc = None, num_observations = nobs))
            regressors.append( glm.regressors.ParametricRegressor(name = 'pside',             values = pside,              preproc = None, num_observations = nobs))
            
            contrasts = list()
            contrasts.append(glm.design.Contrast([ 1, 0, 0, 0, 0], 'correct'))
            contrasts.append(glm.design.Contrast([ 0, 1, 0, 0, 0], 'incorrect'))
            contrasts.append(glm.design.Contrast([ 0, 0, 1, 0, 0], 'confdiff_correct'))
            contrasts.append(glm.design.Contrast([ 0, 0, 0, 1, 0], 'confdiff_incorrect'))
            contrasts.append(glm.design.Contrast([ 0, 0, 0, 0, 1], 'pside'))
            contrasts.append(glm.design.Contrast([ 0, 0,-1, 1, 0], 'confdiff_incorr vs corr'))
            contrasts.append(glm.design.Contrast([ 0, 0, 1, 1, 0], 'confdiff'))
    ...
    #        deepcopy(tl_betas).drop_channels(['RM']).plot_joint(picks = 'eeg', topomap_args = dict(outlines = 'head', contours = 0))
            tl_betas.save(fname = op.join( param['path'], 'glms', 'feedback', glm_folder, 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ lapstr + name + '_betas-ave.fif'))
            del(tl_betas)
    
            tl_tstats = mne.EvokedArray(info = info, nave = nave, tmin = tmin,
                                                      data = np.squeeze(model.get_tstats()[iname,:,:]))
            tl_tstats.save(fname = op.join(param['path'], 'glms', 'feedback', glm_folder, 'wmConfidence_' + param['subid'] + '_feedbacklocked_tl_'+ lapstr + name + '_tstats-ave.fif'))
            del(tl_tstats)
