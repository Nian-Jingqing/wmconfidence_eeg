...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 20 11:49:14 2020
...
mne.grand_average(deepcopy(data['confcorrect'])).plot_joint(times = np.arange(0.05, 0.7, 0.1), title = 'confidence: correct trials')
mne.grand_average(deepcopy(data['confincorrect'])).plot_joint(times = np.arange(0.05, 0.7, 0.1), title = 'confidence regressor: incorrect trials')
mne.grand_average(deepcopy(data['confincorrvscorr'])).plot_joint(times = np.arange(0.05, 0.7, 0.1), title = 'confidence contrast: incorrect vs correct trials')
#%%
...
for channel in ['Fz', 'FCz', 'Cz']:
    mne.viz.plot_compare_evokeds(
            evokeds = dict(
                    correct    = data['confcorrect'],
                    incorrect  = data['confincorrect'],
                    difference = data['confincorrvscorr']),
            colors = dict(
                    correct = '#4daf4a',
                    incorrect = '#e41a1c',
                    difference = '#000000'),
            legend = 'upper right', picks = channel, ci = .68, #show standard error of the ERP at the channel
            show_sensors = False, title = 'confidence regressor: electrode '+channel+' (laplacian)', truncate_xaxis = False)
...
for channel in ['FCz', 'Cz']:
    ...
    mne.viz.plot_compare_evokeds(
            ...
            colors = dict(
                    ...
                    difference = '#000000'),
            legend = 'upper right', picks = channel, ci = .68, #show standard error of the ERP at the channel
            show_sensors = False, title = 'confidence regressor: electrode '+channel+' (laplacian)', ylim = dict(eeg=[-3, 4]),
            axes = ax, truncate_xaxis = False, vlines = [0, 0.5])
    ax.set_title('confidence regressor on the feedback ERP at electrode '+channel+' (laplacian)')
...
fig = mne.grand_average(deepcopy(data['confincorrvscorr'])).plot_joint(topomap_args = dict(contours = 0, vmin = -3, vmax = 3),
...
for channel in ['FCz', 'Cz']:
    ...
    for imask in range(len(tmins)):
        ...
        fig = mne.grand_average(data['confincorrvscorr']).plot_topomap(times = tcentre,
                                                                   average = twidth,
                                                                   vmin = -3, vmax = 3, contours = 0,
                                                                   extrapolate = 'head', res = 300, #dpi
                                                                   title = 'confidence contrast (laplacian): incorr vs corr, %s to %sms\nclu from channel: %s'%(str(np.round(itmin,2)), str(np.round(itmax,2)), channel))
#%% for the confidence regressor in each trial type separately:
...
for channel in ['FCz', 'Cz']:
    ...
    for contrast in ['confcorrect', 'confincorrect']:
        ...
        fig.savefig(fname = op.join(figpath, 'feedbackERP_confidenceRegressor_clustertimesFrom_%s_betas_laplacian.eps'%(channel)), format = 'eps', dpi = 300 )
        fig.savefig(fname = op.join(figpath, 'feedbackERP_confidenceRegressor_clustertimesFrom_%s_betas_laplacian.pdf'%(channel)), format = 'pdf', dpi = 300 )
