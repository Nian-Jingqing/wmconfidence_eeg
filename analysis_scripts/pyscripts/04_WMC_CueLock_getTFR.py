    ...
    for laplacian in [True]:
        ...
        if laplacian:
            for stiff in [3,4]: 
                ...
                slap = mne.preprocessing.compute_current_source_density(cuelocked,
                                                                        stiffness = stiff)
                ...
                tfr.save(fname = param['cuelocked_tfr'].replace('cuelocked-tfr', 'cuelocked_laplacian_stiffness%d-tfr'%(stiff)), overwrite = True)
        else: #dont use surface laplacian, just use normal epoched data for TFR
            print('\nrunning TF decomposition\n')
